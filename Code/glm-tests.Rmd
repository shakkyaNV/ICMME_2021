...
low_acc <- lasso_grid %>% select_best(metric = "roc_auc", penalty)
...
ridge_initial_spec <- linear_reg(penalty = 0.1, 
                                   mixture = 0) %>% 
  set_engine("glmnet") %>% 
  set_mode("regression") %>% 
...
tune_ridge_spec <- linear_reg(penalty = tune(), 
                                mixture = 0) %>% 
  set_engine("glmnet") %>% 
  set_mode("regression") %>% 
  set_args(family = "gaussian")
...
low_roc_ridge <- ridge_grid %>% 
  select_best(metric = "rsq", penalty) 
