...
calibrate with [https://konradsemsch.netlify.app/2019/10/testing-the-tune-package-from-tidymodels-analysing-the-relationship-between-the-upsampling-ratio-and-model-performance/]
...
olim_recipe <- recipe(TF ~., data = split_train) %>% 
  ...
  themis::step_upsample(TF) %>%  # use over_ration = tune() # from yardstick
...
lasso_spec <- logistic_reg(penalty = 0.1, mixture = 1) %>% 
  ...
  # set_args(family = "gaussian") # %>% set_args(type = "response")
...
table(lasso_fit_object$outcomes$notNAProp, split_train$notNAProp)
# huh?
```
